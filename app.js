...
router.post("/users", async (req, res) => {
  try {
    ...
    if (password !== confirmPassword) {
      res.status(400).send({
        errorMessage: "패스워드가 패드워드 확인값과 다릅니다.",
      });
      return;
    }

    // email 혹은 nickname이 동일한 것이 이미 있는지 확인하기 위해 가져온다.
    const existUsers = await User.find({
      $or: [{ email }, { nickname }],
    });
    if (existUsers.length) {
      // NOTE: 보안을 위해 인증 메세지는 자세히 설명하지 않는 것을 원칙으로 한다.
      res.status(400).send({
...
app.listen(3000, () => {
  console.log("서버가 요청을 받을 준비가 되었다");
});
